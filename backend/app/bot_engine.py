...
class BotEngine:
    ...
    Manages:
    - Strategy execution (arb, MM, momentum)
    ...
    async def run_cycle_once(self):
        """Run a single trading cycle (for manual trigger)."""
        await self._cycle()
    ...
    async def _run_arbitrage(self, markets: list[dict]):
        ...
        for signal in signals[:3]:  # Max 3 arb trades per cycle
            ...
            for order in orders:
                ...
                if result["status"] == "filled":
                    ...
                    self.risk.record_order_close(
                        order["size"], result["pnl"], result["volume"]
                    )
                    self.arb.record_trade(result["pnl"])
                    self.airdrop.record_trade(
                        order["market_id"], result["volume"], result["pnl"], "LIMIT"
                    )
                    self._record_trade(result)
                    self.logs.append(
                        "TRADE",
                        f"[ARB] Filled {order['side']} {order['size']:.1f} @ {result['filled_price']:.4f} "
                        f"pnl={result['pnl']:+.4f}",
                        "arbitrage"
                    )
    ...
    async def _run_market_maker(self, markets: list[dict]):
        ...
        # Limit MM to 5 quotes per cycle to avoid spam
        quotes = quotes[:5]
        ...
        for quote in quotes:
            ...
            for order in orders:
                ...
                if result["status"] == "filled":
                    ...
                    self.logs.append(
                        "TRADE",
                        f"[MM] {order['side']} {order['size']:.1f} @ {result['filled_price']:.4f} "
                        f"spread={quote.expected_spread:.3f} pnl={result['pnl']:+.4f}",
                        "market_maker"
                    )
    ...
    async def _run_momentum(self, markets: list[dict]):
        ...
        # Limit to 2 momentum trades per cycle
        signals = signals[:2]
        ...
        for signal in signals:
            ...
            if result["status"] == "filled":
                ...
                self.logs.append(
                    "TRADE",
                    f"[MOM] {signal.direction} {order['size']:.1f} @ {result['filled_price']:.4f} "
                    f"conf={signal.confidence:.2f} pnl={result['pnl']:+.4f}",
                    "momentum"
                )

    def _record_trade(self, result: dict):
        """Add trade to in-memory history."""
        self._trades.append({
